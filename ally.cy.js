describe('Checking new offers with skill tags and applying', () => {
  it('Highlights offers, skill tags, and clicks apply on first fully matching offer', () => {
    cy.visit('https://justjoin.it/job-offers/all-locations/testing');
    ...
    cy.get('div[data-index]', { timeout: 10000 }).should('be.visible').each(($offer) => {
      if (offerCount >= 20 || foundOrangeOffer) return false;

      cy.wrap($offer).within(() => {
        // Sprawdzamy, czy oferta zawiera tag "New"
        cy.get('MuiBox-root css-ai36e1').contains('New').then(() => {
